...
from pbb.models import NNet4l, CNNet4l, ProbNNet4l, ProbCNNet4l, ProbCNNet5lSmall, CNNet5lSmall, ProbCNNet5l, CNNet5l, ProbCNNet9l, CNNet9l, CNNet13l, ProbCNNet13l, ProbCNNet15l, CNNet15l, trainNNet, testNNet, Lambda_var, trainPNNet, computeRiskCertificates, testPosteriorMean, testStochastic, testEnsemble
...
learning_rate_prior=0.01, momentum_prior=0.95, delta=0.025, layers=None, delta_test=0.01, mc_samples=1000, 
...
perc_prior=0.2, batch_size=250, toolarge=False, seed=7):
...
#     np.random.seed(0)
    ...
    if model == 'cnn':
        if name_data == 'cifar10':
            ...
            # with mnist
            if layers == 4:
                net0 = CNNet4lCIFAR(dropout_prob=dropout_prob).to(device)
            elif layers == 4.01:
                net0 = CNNet4lSmallCIFAR(dropout_prob=dropout_prob).to(device)
            elif layers == 5:
                net0 = CNNet5l(dropout_prob=dropout_prob).to(device)
            elif layers == 5.01:
                net0 = CNNet5lSmall(dropout_prob=dropout_prob).to(device)
            elif layers == 9:
    ...
    if model == 'cnn':
        ...
        if name_data == 'cifar10':
            if layers == 4:
                net = ProbCNNet4lCIFAR(rho_prior, prior_dist=prior_dist,
                                    device=device, init_net=net0).to(device)
            elif layers == 4.01:
                net = ProbCNNet4lSmallCIFAR(rho_prior, prior_dist=prior_dist,
                                    device=device, init_net=net0).to(device)
            elif layers == 5:
                net = ProbCNNet5l(rho_prior, prior_dist=prior_dist,
                                    device=device, init_net=net0).to(device)
            elif layers == 5.01:
                net = ProbCNNet5lSmall(rho_prior, prior_dist=prior_dist,
                                    device=device, init_net=net0).to(device)
            elif layers == 9:
    ...
    for epoch in trange(train_epochs):
        ...
        if verbose_test and ((epoch+1) % 5 == 0):
            train_obj, kl, loss_ce_train, err_01_train, empirical_risk_ce, empirical_risk_01, risk_ce, risk_01 = \
                    computeRiskCertificates(net, toolarge, bound, device=device, 
    ...
    print("Computing final risk certificates..., toolarge =", toolarge)
    ...
    print(f"""Objective: {objective}, Dataset: {name_data}, Sigma: {sigma_prior :.5f}, Layers: {layers},
            pmin: {pmin :.5f}, LR: {learning_rate :.5f}, momentum: {momentum :.5f}, 
            LR_prior: {learning_rate_prior :.5f}, momentum_prior: {momentum_prior :.5f}, 
            kl_penalty: {kl_penalty : .5f}, dropout: {dropout_prob :.5f}, 
            Obj_train: {train_obj :.5f}, Risk_CE: {risk_ce :.5f}, Risk_01: {risk_01 :.5f}, KL: {kl :.5f},
            Train xe loss: {loss_ce_train :.5f}, Train 01 loss: {err_01_train},
            Empirical xe risk: {empirical_risk_ce}, Empirical 01 risk: {empirical_risk_01},
            Stch loss: {stch_loss :.5f}, Stch 01 error: {stch_err :.5f}, 
            Post mean loss: {post_loss :.5f}, Post mean 01 error: {post_err :.5f}, 
            Ens loss: {ens_loss :.5f}, Ens 01 error: {ens_err :.5f}, 
            01 error prior net: {errornet0 :.5f}, 
            perc_train: {perc_train :.5f}, perc_prior: {perc_prior :.5f}""")
    ...
    results = {
        ...
        'Risk_01': risk_01, 
        'KL': kl, 
        'Train xe loss': loss_ce_train, 
        'Train 01 loss': err_01_train,
        'Empirical xe risk': empirical_risk_ce, 
        'Empirical 01 risk': empirical_risk_01, 
        'Stch loss': stch_loss.item(), 
        'Stch 01 error': stch_err, 
        'Post mean loss': post_loss.item(), 
        'Post mean 01 error': post_err, 
        'Ens loss': ens_loss.item(), 
        'Ens 01 error': ens_err, 
        '01 error prior net': errornet0, 
