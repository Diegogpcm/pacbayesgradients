...
class CNNet4lSmallCIFAR(nn.Module):
    """Implementation of a standard Convolutional Neural Network with 4 layers
    (used for the experiments on CIFAR so it assumes a specific input size and
    number of classes)
    ...
    def forward(self, x):
        x = self.d(self.conv1(x)) #30x30
        x = F.relu(x)
        x = F.max_pool2d(x, kernel_size=3) #10x10
        x = self.d(self.conv2(x)) #8x8
        x = F.relu(x)
        x = F.max_pool2d(x, kernel_size=2) #4x4
        x = torch.flatten(x, 1)
        x = self.d(self.fc1(x))
        x = F.relu(x)
        x = self.fc2(x)
...
class ProbCNNet4lSmallCIFAR(nn.Module):
    """Implementation of a Probabilistic Convolutional Neural Network with 4 layers
    (used for the experiments on CIFAR so it assumes a specific input size,
    number of classes and kernel size).
    ...
    def __init__(self, rho_prior, prior_dist='gaussian', device='cuda', init_net=None):
        ...
        self.conv1 = ProbConv2d(
            3, 8, 3, rho_prior, prior_dist=prior_dist, device=device, init_layer=init_net.conv1 if init_net else None)
        self.conv2 = ProbConv2d(
            8, 16, 3, rho_prior, prior_dist=prior_dist, device=device, init_layer=init_net.conv2 if init_net else None)
        self.fc1 = ProbLinear(256, 32, rho_prior, prior_dist=prior_dist,
                              device=device, init_layer=init_net.fc1 if init_net else None)
        self.fc2 = ProbLinear(32, 10, rho_prior, prior_dist=prior_dist,
                              device=device, init_layer=init_net.fc2 if init_net else None)
...
class ProbCNNet4lCIFAR(nn.Module):
    """Implementation of a Probabilistic Convolutional Neural Network with 4 layers
    (used for the experiments on CIFAR so it assumes a specific input size,
    number of classes and kernel size).
    ...
    def __init__(self, rho_prior, prior_dist='gaussian', device='cuda', init_net=None):
        ...
        self.conv2 = ProbConv2d(
            32, 64, 3, rho_prior, prior_dist=prior_dist, device=device, init_layer=init_net.conv2 if init_net else None)
        self.fc1 = ProbLinear(2304, 128, rho_prior, prior_dist=prior_dist,
                              device=device, init_layer=init_net.fc1 if init_net else None)
        self.fc2 = ProbLinear(128, 10, rho_prior, prior_dist=prior_dist,
                              device=device, init_layer=init_net.fc2 if init_net else None)
...
class CNNet5lSmall(nn.Module):
    """Implementation of a Convolutional Neural Network with 5 layers
    ...
    def __init__(self, dropout_prob):
        ...
        self.fcl1 = nn.Linear(256, 32)
        self.fcl2 = nn.Linear(32, 10)
        self.d = nn.Dropout2d(dropout_prob)
...
class ProbCNNet5lSmall(nn.Module):
    """Implementation of a Probabilistic Convolutional Neural Network with 5 layers
    (used for the experiments on CIFAR-10 so it assumes a specific input size,
    number of classes, architecture, kernel size, etc.).
    ...
    def __init__(self, rho_prior, prior_dist, device='cuda', init_net=None):
        super().__init__()
        self.conv1 = ProbConv2d(
            in_channels=3, out_channels=8, rho_prior=rho_prior, prior_dist=prior_dist, device=device,
            kernel_size=3, padding=1, init_layer=init_net.conv1 if init_net else None)
        self.conv2 = ProbConv2d(
            in_channels=8, out_channels=16, rho_prior=rho_prior, prior_dist=prior_dist, device=device,
            kernel_size=3, padding=1, init_layer=init_net.conv2 if init_net else None)
        self.conv3 = ProbConv2d(
            in_channels=16, out_channels=16, rho_prior=rho_prior, prior_dist=prior_dist, device=device,
            kernel_size=3, padding=1, init_layer=init_net.conv3 if init_net else None)
        self.fcl1 = ProbLinear(256, 32, rho_prior=rho_prior,
                               prior_dist=prior_dist, device=device, init_layer=init_net.fcl1 if init_net else None)
        self.fcl2 = ProbLinear(32, 10, rho_prior=rho_prior,
...
class ProbCNNet5l(nn.Module):
    ...
    def __init__(self, rho_prior, prior_dist, device='cuda', init_net=None):
        ...
        self.conv2 = ProbConv2d(
            ...
            kernel_size=3, padding=1, init_layer=init_net.conv2 if init_net else None)
        self.conv3 = ProbConv2d(
            in_channels=64, out_channels=64, rho_prior=rho_prior, prior_dist=prior_dist, device=device,
            kernel_size=3, padding=1, init_layer=init_net.conv3 if init_net else None)
        self.fcl1 = ProbLinear(1024, 128, rho_prior=rho_prior,
                               prior_dist=prior_dist, device=device, init_layer=init_net.fcl1 if init_net else None)
        self.fcl2 = ProbLinear(128, 10, rho_prior=rho_prior,
                               prior_dist=prior_dist, device=device, init_layer=init_net.fcl2 if init_net else None)
...
def trainNNet(net, optimizer, epoch, train_loader, device='cuda', verbose=False):
    ...
    if verbose:
        to_load = tqdm(train_loader)
    else:
        to_load = train_loader
        
    for batch_id, (data, target) in enumerate(to_load):
...
def computeRiskCertificates(net, toolarge, pbobj, device='cuda', lambda_var=None, train_loader=None, whole_train=None):
    ...
    with torch.no_grad():
        if toolarge:
            train_obj, kl, loss_ce_train, err_01_train, empirical_risk_ce, empirical_risk_01, risk_ce, risk_01 = pbobj.compute_final_stats_risk(
    ...
    return train_obj, kl, loss_ce_train, err_01_train, empirical_risk_ce, empirical_risk_01, risk_ce, risk_01
